...
const versionOptions = computed(() => [
  { value: '', label: t('shaderPack.allVersions') },
...
async function runSearch() {
  ...
  hasMore.value = true
  try {
    const list = await searchShaderPacks({
...
async function confirmInstall(versionName: string) {
  ...
  installError.value = ''
  try {
    const result = await installShaderPack(installTarget.value.id, versionName)
    installedIds.value.push(installTarget.value.id)
    showInstallPopup.value = false
    if (result.dependencies.length > 0) {
      showToast(t('shaderPack.depsInstalled', { deps: result.dependencies.join(', ') }))
    } else {
      showToast(t('shaderPack.installedOk', { name: result.fileName }))
    }
  } catch (err) {
...
function formatDownloads(n: number): string {
  if (n >= 1e8) return (n / 1e8).toFixed(1) + ' ' + t('shaderPack.unitHundredMillion')
  if (n >= 1e4) return (n / 1e4).toFixed(1) + ' ' + t('shaderPack.unitTenThousand')
  return String(n)
}
...
/** 只保留光影加载器（Iris / OptiFine / Oculus），过滤掉 Modrinth 返回的其他无关标记 */
function shaderLoaders(loaders: string[]): string[] {
  return (loaders || []).filter(l => ['iris', 'optifine', 'oculus'].includes(l))
}
...
<template>
  <div class="rp-content">
    <!-- 筛选框：名称 / MC 版本（光影包不区分加载器，原版即可用） -->
    <div class="filter-bar">
      <input
        type="text"
        v-model="keyword"
        :placeholder="$t('shaderPack.namePlaceholder')"
        class="filter-input keyword-input"
      />
      <AppSelect v-model="versionFilter" :options="versionOptions" />
    </div>

    <!-- 光影包列表（按下载量排序 + 滚动到底自动加载更多） -->
    <div class="rp-list" @scroll="onListScroll">
      <div v-if="loading" class="empty-state">
        <span class="empty-text">{{ $t('shaderPack.searching') }}</span>
      </div>
      <div v-else-if="searchError" class="empty-state">
        <span class="empty-text error-text">{{ $t('shaderPack.searchFailed', { msg: searchError }) }}</span>
      </div>
      <div v-else-if="packs.length === 0" class="empty-state">
        <span class="empty-text">{{ $t('shaderPack.noPacks') }}</span>
      </div>
      <div v-for="pack in packs" :key="pack.id" class="rp-card">
        ...
        <div class="rp-info">
          ...
          <span class="rp-meta">
            {{ t('shaderPack.meta', { count: formatDownloads(pack.downloads), days: daysAgo(pack.updatedAt) }) }}
          </span>
          <span v-if="shaderLoaders(pack.loaders).length" class="rp-loaders">
            <span
              v-for="l in shaderLoaders(pack.loaders)"
              :key="l"
              class="loader-badge"
              :class="l"
            >{{ $t('shaderPack.loaderLabels.' + l) }}</span>
          </span>
        </div>
        ...
        >
          ...
          <span>{{ installedIds.includes(pack.id) ? $t('shaderPack.installed') : $t('shaderPack.download') }}</span>
        </button>
      </div>
      <div v-if="loadingMore" class="list-footer">
        ...
        <span>{{ $t('shaderPack.loadingMore') }}</span>
      </div>
      <div v-else-if="!hasMore && packs.length > 0" class="list-footer">
        <span>{{ $t('shaderPack.endOfList') }}</span>
      </div>
    </div>
      ...
      <div class="popup-card">
        ...
        <div class="popup-body">
          ...
          >
            ...
            <span
              v-if="['iris', 'optifine', 'oculus'].includes(v.loader)"
              class="loader-badge"
              :class="v.loader"
            >{{ $t('shaderPack.loaderLabels.' + v.loader) }}</span>
            <button
            ...
            >
              ...
              <span v-else>{{ $t('shaderPack.install') }}</span>
            </button>
          </div>
...
.rp-loaders .loader-badge.iris {
  background: rgba(78, 168, 222, 0.25);
  color: var(--text-color);
}
.rp-loaders .loader-badge.optifine {
  background: rgba(240, 196, 25, 0.22);
  color: var(--text-color);
}
.rp-loaders .loader-badge.oculus {
