...
> **本文档是项目开发期的 AI 辅助上下文说明**，用于让 AI 编程助手（Cursor、Copilot 等）生成符合规范的前端代码。
> 面向用户的项目说明请见仓库根目录 [README.md](../README.md)。
>
> ⚠️ 注意：本文档编写于项目早期，部分内容已过时（例如文中提到的 `AppHeader.vue`、`LogDisplay.vue` 现已不存在，组件数量已从 6 个增长到 24 个）。实际以 `src/` 目录为准。
...
3. 前端代码位于 `src/` 目录，后端 Rust 代码位于 `src-tauri/`。
